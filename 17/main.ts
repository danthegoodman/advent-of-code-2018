...
function renderGrid(grid: Material[][], scale: number) {
...
async function drawWater(x: number, y: number, frameDelay: number): Promise<void> {
  ...
  if (frameDelay < 20) {
    if (subFrameDelay < 20) {
      ...
      await new Promise<void>(resolve => setTimeout(resolve, subFrameDelay));
      subFrameDelay = 0;
    }
  } else {
...
function drawStillwater(x: number, y: number) {
  $ctx.fillStyle = "#4169E1";
  $ctx.fillRect(x, y, 1, 1);
}

async function run(input: string, scale: number) {
  ...
  async function fillFromDownspout(x: number, y: number) {
    if (grid[x][y] === WATER) return;
    if (grid[x][y] === STILLWATER) return;
  ...
  async function fillDown(x: number, y: number) {
    while (true) {
      grid[x][y] = WATER;
      ...
      if (next === CLAY) break;
      if (next === STILLWATER) break;
      y++;
    }
    return y;
  }

  async function fillSideways(sourceX: number, sourceY: number) {
  ...
  function makeStillWater(startX: number, y: number) {
    for (let xDelt of [-1, +1]) {
      let x = startX;
...
function countWaters(grid: Material[][]) {
  let water = 0;
  let stillwater = 0;
  for(let row of grid){
    for(let it of row){
      if(it === WATER) water ++;
      if(it === STILLWATER) stillwater ++;
    }
  }
