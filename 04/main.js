...
function main() {
    const input = fs_1.readFileSync(__dirname + "/input.txt", 'utf8').trim();
    ...
    chai_1.expect(solveA(example)).to.equal(240);
    console.log("A:", solveA(input));
    chai_1.expect(solveB(example)).to.equal(4455);
...
function buildSleepyTimes(logs) {
    let sleepyTimes = [];
    let guard = -1;
    let sleepStart = -1;
    for (let l of logs) {
